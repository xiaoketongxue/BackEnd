...
## Linux基本目录结构
  包含系统启动必须，指令集合，外部文件管理，临时文件，账户，运行过程中要用，扩展用的
### 系统启动必须：
...
## 磁盘管理
### 内存概念
计算机的存储器有两类，一类是**内部存储器**，一断电就会把记住的东西忘光，一类是**外部存储器**，断了电也能记住。
* 内存：存储信息速度快，断电后存储内容全部丢失。也叫**物理内存。
* 外部存储器呢，主要是磁盘，它所存储的信息不受断电的影响，但是它的速度相对于内存就慢得多了。
    * 软盘：计算机上有个特殊的地方叫做软盘驱动器，要用软盘的时候就把它放进这个地方，不用的时候可以很方便地拿出来带走。
    * 硬盘：硬盘在机箱里面负责储存数据，而软盘用来搬运数据，硬盘的容量大，软盘的容量小，这就是它们的区别，另外硬盘的存取速度比软盘快得多。
* **虚拟内存**是指使用**磁盘**当作RAM的扩展
* 交换空间（Swap space）在物理内存（RAM）被充满时被使用。如果系统需要更多的内存资源，而物理内存已经充满，**内存中不活跃的页**就会被移到交换空间去。虽然交换空间可以为带有少量内存的机器提供帮助，但是这种方法不应该被当做是对内存的取代。**交换空间位于硬盘驱动器上**，它比进入物理内存要慢

### 指令
* ls   不仅可以查看linux文件**包含的文件**，而且可以查看**文件权限**；其中，ll是“ls -l"的别名     
...
* df 显示磁盘分区上可以使用的磁盘空间
    ...
    * 文件-h, --human-readable 使用人类可读的格式(K,M,G)
    * (disk free) df -h
...
* whereis指令只能用于查找二进制文件、源代码文件和man手册页，一般文件的定位需使用locate命令。
*  find 命令用来在指定目录下查找文件。任
...
### Inode节点
* Linux操作系统引进了一个非常重要的概念inode，中文名为**索引结点**，引进索引接点是为了在**物理内存上找到文件块**，所以inode中包含文件的相关基本信息，比如文件位置、文件创建者、创建日期、文件大小等待，输入**stat指令可以查看某个文件的inode信息；
* 硬盘格式化的时候，操作系统自动将硬盘分成两个区域，**一个是数据区，一个是inode区**，存放inode所包含的信息，查看每个硬盘分区的inode总数和已经使用的数量，可以用df命令；
* 在linux系统中，系统内部并不是采用文件名查找文件，而是**使用inode编号来识别文件**。查找文件分为三个过程：系统找到这个文件名对应的**inode号码**，通过inode号码获得**inode信息**，根据inode信息找到文件数据所在的**block读取数据；
* 除了文件名之外的所有文件信息，都存储在inode之中。

### 软连接、硬链接
* 软链接可以看作是Windows中的快捷方式，可以让你快速链接到目标档案或目录。硬链接则透过文件系统的inode来产生新档名，而不是产生新档案。
* 软链接（符号链接） ln -s   source  target（不在同一个目录下source使用绝对路径）    
...
* 软链接(soft link)：A是B的软链接（A和B都是文件名），A的目录项中的inode节点号与B的目录项中的**inode节点号不相同，A和B指向的是两个不同的inode，继而指向两块不同的数据块。但是A的数据块中存放的只是B的路径名（可以根据这个找到B的目录项）**。A和B之间是“主从”关系，如果B被删除了，A仍然存在（因为两个是不同的文件），但指向的是一个无效的链接。
* 硬链接：不能对目录创建硬链接；不能对不同的文件系统创建硬链接；不能对不存在的文件创建硬链接；
* 软连接：可以对目录创建软连接；可以跨文件系统；可以对不存在的文件创建软连接；
* 因为链接文件包含有原文件的路径信息，所以当原文件从一个目录下移到其他目录中，再访问链接文件，系统就找不到了，而硬链接就没有这个缺陷，你想怎么移就怎么移；还有它要系统分配额外的空间用于建立新的索引节点和保存原文件的路径。

## 文本处理工具
* grep：是一种强大的**文本搜索工具**，它能使用**正则表达式搜索文本，并把匹配的行打印出来。常用来在结果中搜索特定的内容。  
        grep     匹配条件   处理文件       
* sed：更侧重对搜索**文本的处理**，如修改、删除、替换等等。sed主要用来自动编辑一个或多个文件；简化对文件的反复操作；编写转换程序等。
* awk：awk是一个强大的**文本分析工具**，相对于grep的查找，sed的编辑，awk在其对数据分析并生成报告时，显得尤为强大。简单来说awk就是把文件(或其他方式的输入流, 如重定向输入)逐行的读入（看作一个记录集）, 把每一行看作一条记录，以空格(或\t,或用户自己指定的分隔符)为默认分隔符将每行切片（类似字段），切开的部分再进行各种分析处理。
...
*  Linux查看某个进程的线程 
    ...
    * top -H
    *  Htop
    * 杀死指定的线程ps -ef|grep '**XXXXXX**'|grep -v 'grep'|awk '{print $2}'|xargs kill -9
* 管道|
