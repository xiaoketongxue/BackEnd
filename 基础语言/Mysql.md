...
## 事务的隔离级别
* 读未提交。事务中的修改，即使没有提交，其他事务也可以看得到最低的隔离级别。**会产生脏读，不可重复读，幻读问题
* 读提交。这种隔离级别就是一个事务的开始，只能看到已经完成的事务的结果，正在执行的，是无法被其他事务看到的。**会产生不可重复读，幻读问题
* 可重复读。该级别保证了每行的记录的结果是一致的，但是却不能保证没有插入新的数据。**会产生幻读问题
* 串行化。最高的隔离级别，在这个隔离级别下，不会产生任何异常。并发的事务，就像事务是在一个个按照顺序执行一样。
         由于他大量加上锁，导致大量的请求超时，**因此性能会比较低下**，再特别需要数据一致性且并发量不需要那么大的时候才可能考虑这个隔离级别。     
